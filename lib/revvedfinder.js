...
RevvedFinder.prototype.getCandidatesFromMapping = function (file, searchPaths, absolute) {
  ...
  searchPaths.forEach(function (sp) {
    ...
    if (self.mapping[key]) {
      ...
      if (absolute) candidates.push(self.mapping[key]);
      else candidates.push(filepath + cfile);
...
RevvedFinder.prototype.getCandidatesFromFS = function (file, searchPaths, absolute) {
  ...
  var self = this;
  if (absolute) {
    regSuffix = '(' + regexpQuote(basename) + regexpQuote(extname) + ')';
    revvedRx = new RegExp(regSuffix);
  }
  ...
  searchPaths.forEach(function (sp) {
    ...
    var files;
    if (absolute) {
      searchString = path.join(sp, dirname, basename + extname);
      files = self.expandfn([searchString]);
    } else {
      files = self.expandfn([searchString, prefixSearchString]);
    }
...
//
RevvedFinder.prototype.getRevvedCandidates = function (file, searchPaths, absolute) {
  var candidates;
  ...
  if (this.mapping) {
    ...
    candidates = this.getCandidatesFromMapping(file, searchPaths, absolute);
  } else {
    debug('Looking on disk');
    candidates = this.getCandidatesFromFS(file, searchPaths, absolute);
  }
...
//
RevvedFinder.prototype.find = function find(ofile, searchDirs, fromFile, returnAbsolute) {
  var file = ofile;
  ...
  var filepaths = this.getRevvedCandidates(file, searchPaths, returnAbsolute);
