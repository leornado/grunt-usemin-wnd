...
//
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath, fromFile, skipThisFile) {
  ...
  var srcfromFilePath = fromFile.dir + '/' + fromFile.name;
  var fromFilePath = self.finder.find(srcfromFilePath, assetSearchPath, fromFile, true);
  ...
  regexps.forEach(function (rxl) {
    ...
    var replaceFilter = rxl[4] || identity;
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      var fromIndex = fromFilePath.indexOf(file);
      if (skipThisFile && fromIndex > 0 && fromIndex == fromFilePath.length - file.length) {
        return match;
      } else {
        var res = replaceFilter(match.replace(src, filterOut(file)));
        if (srcFile !== file) {
...
FileProcessor.prototype.process = function (filename, assetSearchPath, skipThisFile) {
  ...
  var content = this.replaceWithRevved(this.replaceBlocks(this.file), this.file.searchPath, this.file, skipThisFile);

  return content;
};

FileProcessor.prototype.findDependencies = function findDependencies(lines, assetSearchPath, fromFile) {
  // Replace script sources
  var self = this;
  var content = lines;
  var regexps = this.patterns;
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      // Consider reference from site root
      var srcFile = filterIn(src);
      var file = self.finder.find(srcFile, assetSearchPath, fromFile, true);
      if (src === '/') {
        return match;
      }
      if (src !== file) {
        if (!file || src == 'undefined' || file.indexOf('undefined') >= 0)
          return match;
        ...
        if (fileExt && srcExt != fileExt) {
          basename += fileExt;
        }

        var _srcFile = dirname + '/' + basename;
        if (!dependencies) {
          dependencies = {};
          dependencies[srcfromFilePath] = {src: fromFilePath};
        }
        dependencies[file] = {src: _srcFile};
      }
    });
  });
...
FileProcessor.prototype.scanDependencies = function (filename, assetSearchPath) {
  debug('scan dependencies of file %s', filename, assetSearchPath);
  ...
  return this.findDependencies(this.replaceBlocks(this.file), this.file.searchPath, this.file);
};
